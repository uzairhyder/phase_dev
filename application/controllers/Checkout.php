...
class Checkout extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['banner'] = $this->model_inner_banner->get_banner(11);
    ...
    public function step3()
    {
        ...
        if ($this->userid <= 0) {
            ...
            // Order Completed
            if ((!array_filled($orderDetail)) || ($orderDetail['order_payment_comments'] == 'Completed')) {
                redirect(g('base_url'));
            }
    ...
    public function success()
    {
        if($this->userid <= 0){
            ...
            if($this->session->userdata('is_success')==null){
                redirect(g('base_url'));
            }

        $data['banner'] = $this->model_inner_banner->get_banner(20);

        $data['heading'] = "Success Payment";

        $data['text'] = "Thank you! Your payment has been Completed.";
    ...
    public function notify()
    {
        ...
        if ($code == md5($oid)) {
            ...
            if ($updateParam['order_payment_status'] == 1) {
                ...
                // Send exam email and update ACCESS CODE
                $this->exam_email($order_id);

                //if ((ENVIRONMENT == 'testing') || (ENVIRONMENT == 'production') || (ENVIRONMENT == 'development')) {
...
//
    ...
    public function stripe_payment()
    {
        ...
        if (isset($post) && array_filled($post)) {
            ...
            if ($order_payment_status > 0) {
                ...
                redirect(l('checkout/step2?oid=' . $order_id . "&msgtype=error&msg=" . $title), true);
            }
        }
        else{
            redirect('/');
    ...
    public function add_cart()
    {
        ...
        if(count($data_post) > 0)
        {
            ...
            /*if($data_post['wishlist'] == 1){
                $this->model_wishlist->remove_to_wishlist(intval($data_post['product_id']));
            }*/

            if($this->check_id_exist_cart($data_post['product_id'], $data_post['type'])){
                $results['status'] = 0;
                $results['msg'] = 'Item is already added';
            }
            else{
...
//                                );
                ...
                $price = $pro_price = $product_data['product_price'];
                //$pricing_info = $this->model_pricing->find_by_pk($data_post['price_id']);

                /*if($pricing_info['pricing_type']=='1'){
                    $text = "Days";
                }
                elseif($pricing_info['pricing_type']=='2'){
                    $text = "Month";
                }
                elseif($pricing_info['pricing_type']=='3'){
                    $text = "Year";
                }*/

                //$price = $pro_price = $pricing_info['pricing_amount'];
                ...
                $addons = array();
                if(!empty($data_post['addon'])){
                    $ids = explode(',',$data_post['addon']);
                    //debug($ids);
                    foreach($ids as $key=>$value):
                        $add_ids['addons_id'][] = $value;
                    endforeach;
                    //debug($add_ids,1);
                    $params['where_in'] = $add_ids;
                    $addons = $this->model_addons->find_all_active($params);

                    $price = $price + array_sum(array_column($addons,'addons_price'));
                }
                ...
                $data = array(
                    ...
                    'options' => array(
                        ...
                        'product_color_id'=>$product_data['color'],
                        //'price_id'=>$data_post['price_id'],
                        //'price_number'=>$pricing_info['pricing_number'],
                        //'price_text'=>$text,
                        //'product_price'=>$pro_price,
                        'product_price'=>$price,
                        'product_slug' => g('base_url')."product/detail/".$product_data['product_slug'],
                        //'product_gamertag' => $data_post['Gamertag'],
                        //'product_dropdown' => (count($data_post['dropdown'])>0 ? serialize($data_post['dropdown']) : ''),
                        //'product_playwith' => $data_post['playwith'],
                        //'product_color' => $data_post['color'],
                        //'product_pound' => $pounds,
                        'product_img' => Links::img($product_data['product_image_path'],$product_data['product_image']),
                        'product_img_thumb' => Links::img($product_data['product_image_path'],$product_data['product_image_thumb']),
                        //'addons'=> $addons
                        //'kit'=> (isset($data_post['kit_id'])) ? $data_post['kit_id'] : 0,
                        //'prep_size'=> (isset($data_post['prep_size_id'])) ? $data_post['prep_size_id'] : 0
...
//                                );
    ...
    public function update_qty_cart()
    {
        ...
        if (count($input) > 0) {
            ...
            $product_id = $this->cart->contents()[$input['id']]['id'];
            $params['fields'] = "product_stock";
            $product_info = $this->model_product->find_by_pk($product_id, false,$params);
            $data['rowid'] = $input['id'];
            $data['qty'] = $input['qty'];
            $this->cart->update($data);
            echo json_encode(array('status' => 1,));
            /*if($product_info['product_stock']>$input['qty']){
                $data['rowid'] = $input['id'];
                $data['qty'] = $input['qty'];
                $this->cart->update($data);
                echo json_encode(array('status' => 1,));
    ...
    public function save_order()
    {
        ...
        if ($this->validate("model_order", $custom_rule)) {
        // debug(1,1);
            $signupID = 0;
            $form_name = 'order';
            ...
            $contact_us_data['order_amount'] = $this->total_amount + $this->tax;
            $contact_us_data['order_total'] = $this->total_amount;
            $contact_us_data['order_tax'] = $this->tax;
            ...
            if ($this->discount > 0) {
                $contact_us_data['order_coupon_amount'] = $this->discount;
                $contact_us_data['order_coupon_id'] = intval($this->session->userdata['discount']['coupon']['coupon_id']);;
    ...
    function fedex_values($data = '')
    {
        ...
        try {
            ...
            if ($response->HighestSeverity != 'FAILURE' && $response->HighestSeverity != 'ERROR') {
                ...
                $paramopt['total'] = price($this->cart->total() + $amount);

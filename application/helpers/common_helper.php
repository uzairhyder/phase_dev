 ...
 * Image url
 **/
if( ! function_exists('get_image_2') )
{
    function get_image_2($image_path,$image_name)
    {
        global $config;
        if ($image_name == '') {
            $data = $config['base_url'].'assets/front_assets/images/logo.png';
        }
        else{
            $data = $config['base_url'].$image_path.$image_name;
        }
        return $data;
    }
}

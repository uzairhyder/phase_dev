...
<div class="inner-page-header">
	...
	if($class_name == 'kicker_year'){?>
		<h1><?=humanize('Kickoffs Years')?> <small>Listing</small></h1>
	<?}else if($class_name == 'kicker_ranking'){?>
		<h1><?=humanize('Kickoffs Ranking')?> <small>Listing</small></h1>
	<?}else{?>
				...
				<div class="actions">
                    ...
                    <?if(!$dt_params['action']['hide_add_button']){?>
						<a href="<?=$config['base_url']?>admin/<?=$class_name?>/add" class="btn default yellow-stripe">
							...
							Add New <?=humanize('Kickoffs Years')?> </span>
							<?}else if($class_name == 'kicker_ranking'){?>
								<span class="hidden-480">
							Add New <?=humanize('Kickoffs Ranking')?> </span>
							<?}else{?>
								<span class="hidden-480">
